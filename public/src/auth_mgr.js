...
function buildHTML(){
    ...
    let auth_menu = $("<div style='position:fixed;top:1.5rem;right:2.5rem;margin-left:auto;margin-right:0px;background-color:white;border:1px solid gray;border-radius:1.5rem;filter:drop-shadow(0 0 0.75rem gray);' id='auth-menu'></div>");

    let auth_menu_icon_holder = $("<div id='auth-menu-icon-holder' style='display:flex;flex-direction:row-reverse;border-radius:1.5rem;background-color: white;padding:1px;'></div>");
    ...
    let log_in_form = $("<div style='padding:1rem;' class='d-none' id='log_in_form'></div>");
    let sign_out_form = $("<div class='d-none' id='sign_out_form' style='padding:1rem;'></div>");
    let sign_up_form = $("<div class='d-none' id='sign_up_form' style='padding:1rem;'></div>");

    let message_zone = $("<div style='padding:1rem;' id='auth-mgr-msg-zone' class='d-none'></div>");
    ...
    log_in_form.append(`<p>Log In</p><div>
            ...
            <button class="btn btn-outline-secondary" id="input-login-submit">Log In</button>
        </div>`);
...
function initialize_auth(optionalOnChangeFunc = undefined){
    ...
    console.log("running the initialize auth method.");
